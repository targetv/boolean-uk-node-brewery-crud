...
breweriesRouter.get("/", (req, res) => {
  ...
  if (breweryType) {
    const filterBreweryType = breweries.filter(
      (brewery) => brewery.brewery_type === breweryType
    );
    const response = filterBreweryType.length
      ? res.json(filterBreweryType)
      : "Cant find brewerys";
    res.json(response);
  } else {
    res.json(breweries);
...
breweriesRouter.delete("/brewery/:id", (req, res) => {
  const breweryId = req.params.id;
  breweries = breweries.filter((brewey) => brewey.id !== breweryId);
  res.json({ newBreweries: breweries });
});

breweriesRouter.post("/addBrewery", (req, res) => {
  // need to have data to create a new product
  const newProduct = req.body;
  breweries = [...breweries, newProduct];
